...
export function formatHelp(botUsername: string): string {
  return [
    "<b>Поиск кандзи</b>",
    "",
    "<b>1. В личке с ботом</b> — просто напиши запрос:",
    "• <code>вода</code>, <code>мизу</code>, <code>mizu</code>, <code>水</code>",
    "",
    `<b>2. В любом чате</b> (в т.ч. Избранное) — <code>@${escapeHtml(botUsername)}</code> и запрос:`,
    `• <code>@${escapeHtml(botUsername)} вода</code>`,
    "",
    "В списке — превью. После выбора — карточка с анимацией (если есть).",
    "",
